...
INITIALIZATION
  ...
  registerRule(IPC_NAMESPACE,'startIpcServer'      ,@startIpcServer_impl      ,[se_writingInternal],ak_binary ,'startIpcServer(id:string,serve:expression(1));//Creates an IPC server');
  ...
  registerRule(IPC_NAMESPACE,'isIpcServerRunning'  ,@isIpcServerRunning_impl  ,[se_readingInternal],ak_unary  ,'isIpcServerRunning(serverId:string);//Returns true if the given IPC server is running and false otherwise');
FINALIZATION
  registry.destroy;
