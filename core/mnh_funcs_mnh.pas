...
FUNCTION sleep_imp intFuncSignature;
  ...
  begin
    ...
    if (params<>nil) and (params^.size=1) and (arg0^.literalType in [lt_real,lt_smallint,lt_bigint]) and context.checkSideEffects('sleep',tokenLocation,[se_sleep]) then begin
      ...
      while (context.wallclockTime(true)<sleepUntil) and (context.threadLocalMessages.continueEvaluation) do begin
    ...
    begin
      case x^.literalType of
        ...
        lt_error,lt_void, lt_real: begin
          context.threadLocalMessages.raiseError('ord can only be applied to booleans, ints and strings',tokenLocation);
          exit(newVoidLiteral);
        end else begin
          ...
          for sub in iter do if context.threadLocalMessages.continueEvaluation then
