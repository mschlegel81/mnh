    ...
    if (params<>nil) and (params^.size=1) and (arg0^.literalType=lt_string) and context.checkSideEffects('readXmlFile',tokenLocation,[se_readFile]) then begin
      ...
      then try
        ...
        on e:Exception do begin
          context.threadLocalMessages.raiseError('Error parsing XML file '+str0^.value+': '+e.message,tokenLocation);
        end;
      end else begin
        context.threadLocalMessages.postTextMessage(mt_el2_warning,tokenLocation,'XML File '+str0^.value+' does not exist');
        exit(newVoidLiteral);
      end;
      result:=obtainXmlData(FDoc);
      if result=nil then begin
        context.threadLocalMessages.postTextMessage(mt_el2_warning,tokenLocation,'Error parsing XML file '+str0^.value);
  ...
  begin
    ...
    if (params<>nil) and (params^.size=1) and (arg0^.literalType=lt_string) then begin
      ...
      try
        ...
        on e:Exception do begin
          context.threadLocalMessages.raiseError('Error parsing XML input: '+e.message,tokenLocation);
        end;
      end;
      FreeAndNil(input);
      result:=obtainXmlData(FDoc);
      if result=nil then begin
        context.threadLocalMessages.raiseError('Error parsing XML input.',tokenLocation);
        result:=newVoidLiteral;
      end;
    end;
  end;
