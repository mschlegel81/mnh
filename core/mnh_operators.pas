  ...
  begin
    ...
    if (rule<>nil) then begin
      if P_threadContext(context)^.callDepth>=STACK_DEPTH_LIMIT then begin
        P_threadContext(context)^.threadLocalMessages.raiseError('Stack overflow in overridden operator',tokenLocation,mt_el4_systemError);
        exit(newVoidLiteral);
      end;
    ...
    if result=nil then begin
      P_threadContext(context)^.threadLocalMessages.raiseError('Incompatible operators '+LHS^.typeString+' and '+RHS^.typeString+' for operator '+C_tokenInfo[op].defaultId,tokenLocation);
  ...
  begin
    ...
    if (rule<>nil) then begin
      if context.callDepth>=STACK_DEPTH_LIMIT then begin
        context.threadLocalMessages.raiseError('Stack overflow in overridden comparator',tokenLocation);
        exit(nil);
      end;
    ...
    then begin
      ...
      if result=nil then context.threadLocalMessages.raiseError('Incompatible comparands '+arg0^.typeString+' and '+arg1^.typeString,tokenLocation);
...
FUNCTION outerFunc_id intFuncSignature;
  ...
  begin
    ...
    if (rule<>nil) then begin
      if context.callDepth>=STACK_DEPTH_LIMIT then begin
        context.threadLocalMessages.raiseError('Stack overflow in overridden operator',tokenLocation);
        exit(nil);
      end;
      inc(context.callDepth);
      if rule^.replaces(tt_localUserRule,tokenLocation,params,ruleOut,dummy,@context) then begin
    ...
    then begin
      result:=function_id(arg0,arg1,tokenLocation,context);
      if result=nil then context.threadLocalMessages.raiseError('Incompatible operators '+arg0^.typeString+' and '+arg1^.typeString+' for operator '+C_tokenInfo[op].defaultId,tokenLocation);
    end else if (params<>nil) and (params^.size=1)
    then exit(arg0^.rereferenced)
    else if (params=nil) or (params^.size=0) then exit(newVoidLiteral);
  end}
      ...
      lt_smallint: case RHS^.literalType of
        ...
        lt_bigint: if P_bigIntLiteral(RHS)^.value.canBeRepresentedAsInt32
                   then exit(pot_int_int(P_smallIntLiteral(LHS)^.value,P_bigIntLiteral(RHS)^.value.toInt))
                     ...
                     context.threadLocalMessages.raiseError('Huge exponents are unimplemented',tokenLocation);
                     exit(newVoidLiteral);
                   end;
