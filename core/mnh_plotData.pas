...
CONST
  ...
  = ((idx: C_lineStyle_straight; Name: ('line', 'l')),
...
type
  ...
  T_sampleRow = object(T_serializable)
    ...
    PROCEDURE setRules(CONST forXOrNil, forY: P_expressionLiteral;
      CONST t0, t1: double; CONST maxSamples: longint);
...
FUNCTION addPlot(CONST params: P_listLiteral; CONST tokenLocation: T_tokenLocation;
  CONST callDepth: word): P_literal;
FUNCTION setAutoscale(CONST params: P_listLiteral; CONST tokenLocation: T_tokenLocation;
  CONST callDepth: word): P_literal;
FUNCTION setLogscale(CONST params: P_listLiteral; CONST tokenLocation: T_tokenLocation;
  CONST callDepth: word): P_literal;
FUNCTION setPlotRange(CONST params: P_listLiteral; CONST tokenLocation: T_tokenLocation;
  CONST callDepth: word): P_literal;
FUNCTION setAxisStyle(CONST params: P_listLiteral; CONST tokenLocation: T_tokenLocation;
  CONST callDepth: word): P_literal;
FUNCTION setPreserveAspect(CONST params: P_listLiteral;
  CONST tokenLocation: T_tokenLocation; CONST callDepth: word): P_literal;
...
PROCEDURE T_sampleRow.setRules(CONST forXOrNil, forY: P_expressionLiteral;
  CONST t0, t1: double; CONST maxSamples: longint);
  begin
...
FUNCTION T_plot.loadFromFile(VAR F: T_File): boolean;
  ...
  begin
    ...
    iMax := f.readLongint;
    if not (F.allOkay) or (iMax < 0) then
      exit(False);
    Clear;
    setLength(row, iMax);
    result := True;
...
PROCEDURE T_plot.setScreenSize(CONST Width, Height: longint);
  ...
  PROCEDURE initTics(CONST axis: char);
    ...
    PROCEDURE addTic(CONST realPos: double; CONST realTxt: ansistring;
    CONST isMajorTic: boolean);
      VAR
        screenPos: double;
      begin
        screenPos := realToScreen(axis, realPos);
        if not (IsNan(screenPos)) and not (IsInfinite(screenPos)) and
          not (screenPos < 0) and not ((axis = 'y') and
...
FUNCTION getAutoscale(CONST params: P_listLiteral; CONST tokenLocation: T_tokenLocation;
  CONST callDepth: word): P_literal;
  begin
...
FUNCTION getLogscale(CONST params: P_listLiteral; CONST tokenLocation: T_tokenLocation;
  CONST callDepth: word): P_literal;
  begin
  ...
  begin
    ...
    if (params <> nil) and (params^.size = 1) and
      ...
      if (x^.literalType in [lt_intList, lt_realList, lt_numList]) and
        ...
        if not (IsNan(x0)) and not (IsInfinite(x0)) and not (IsNan(x1)) and
          not (IsInfinite(x1)) and not (IsNan(y0)) and not (IsInfinite(y0)) and
          not (IsNan(y1)) and not (IsInfinite(y1)) then
          begin
...
FUNCTION getPlotRange(CONST params: P_listLiteral; CONST tokenLocation: T_tokenLocation;
  CONST callDepth: word): P_literal;
  begin
...
FUNCTION getAxisStyle(CONST params: P_listLiteral; CONST tokenLocation: T_tokenLocation;
  CONST callDepth: word): P_literal;
  begin
...
FUNCTION getPreserveAspect(CONST params: P_listLiteral;
  CONST tokenLocation: T_tokenLocation; CONST callDepth: word): P_literal;
  begin
    result := activePlot.getPreserveAspect;
  end;

FUNCTION renderToFile_impl(CONST params: P_listLiteral;
  CONST tokenLocation: T_tokenLocation; CONST callDepth: word): P_literal;
  VAR
