...
TYPE
  ...
  T_plot = object
    ...
    private
      ...
      PROCEDURE drawCoordSys(CONST target: TCanvas; CONST intendedWidth,intendedHeight:longint);
...
PROCEDURE T_plot.drawCoordSys(CONST target: TCanvas; CONST intendedWidth,intendedHeight:longint);
  VAR i, x, y: longint;
      cSysX,cSysY:longint;
  begin
...
PROCEDURE T_plot.renderPlot(VAR plotImage: TImage; CONST supersampling:longint);
  ...
  begin
    ...
    try
      ...
      drawCoordSys(plotImage.Canvas,plotImage.width,plotImage.height);
    finally
      system.leaveCriticalSection(cs);
    end;
  end;
...
FUNCTION T_plot.obtainPlot(CONST width,height,supersampling:longint):TImage;
  ...
  begin
    ...
    try
      ...
      scalingOptions.updateForPlot(renderImage.Canvas,width,height,row,tic);
      drawGridAndRows(renderImage.Canvas,supersampling);
      ...
      drawCoordSys(result.Canvas,width,height);
    finally
      system.leaveCriticalSection(cs);
    end;
  end;
