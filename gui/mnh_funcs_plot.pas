  ...
  begin
    ...
    if (params<>nil) and (params^.size>=1) then begin
      ...
      if (sizeWithoutOptions = 1) and (arg0^.literalType in [lt_list,lt_intList, lt_realList, lt_numList]) then begin
        context.globalMessages^.postCustomMessage(addRowMessage(newDataRow(list0)));
        exit(newVoidLiteral);
      end;
      if (sizeWithoutOptions = 2) and
        ...
        context.globalMessages^.postCustomMessage(addRowMessage(newDataRow(list1,list0)));
        exit(newVoidLiteral);
      end;
      if (sizeWithoutOptions = 4) and
        ...
        context.globalMessages^.postCustomMessage(addRowMessage(generateRow(P_expressionLiteral(arg0),
                                                                fReal(arg1),
                                                                fReal(arg2),
                                                                int3^.intValue,
                                                                tokenLocation,
...
FUNCTION plot intFuncSignature;
  begin
    ...
    context.globalMessages^.postSingal(mt_plot_clear,C_nilTokenLocation);
...
FUNCTION getOptions intFuncSignature;
  ...
  begin
    ...
    if (params=nil) or (params^.size=0) then begin
      opt:=getOptionsViaAdapters(context.threadLocalMessages);
...
FUNCTION setOptions intFuncSignature;
  ...
  PROCEDURE matchKey(CONST key:string; CONST value:P_literal);
    PROCEDURE fail;
      begin
        context.globalMessages^.postTextMessage(mt_el2_warning,tokenLocation,
  ...
  begin
    ...
    if allOkay then begin
      ...
      context.globalMessages^.postCustomMessage(postOptionsMessage);
    end;
  end;
  ...
  begin
    ...
    if (params<>nil) and (params^.size>=3) and
      ...
      try
        ...
        context.globalMessages^.postCustomMessage(renderRequest);
      except
        on e:Exception do begin
          context.threadLocalMessages.raiseError('Error on renderToFile: '+e.message,tokenLocation);
          exit(nil);
        end;
      end;
      result:=newVoidLiteral;
...
FUNCTION renderToString_impl intFuncSignature;
  ...
  begin
    ...
    if (params<>nil) and (params^.size>=2) and
      ...
      context.globalMessages^.postCustomMessage(renderRequest^.rereferenced);
      result:=newStringLiteral(renderRequest^.getStringWaiting(context.threadLocalMessages));
...
FUNCTION removePlot_imp intFuncSignature;
  ...
  begin
    ...
    if (params=nil) or (params^.size=0) or
      ...
      context.globalMessages^.postCustomMessage(dropPlotMessage);
      result:=newVoidLiteral;
    end else result:=nil;
  end;
...
FUNCTION drawTextRelativeOrAbsolute(CONST params:P_listLiteral; CONST tokenLocation:T_tokenLocation; VAR context:T_threadContext; CONST abspos:boolean):P_literal;
  ...
  begin
    ...
    if (params<>nil) and (params^.size>=3) and
      ...
      context.globalMessages^.postCustomMessage(postRequest);
      result:=newVoidLiteral;
    end;
  end;
