  ...
  begin
    ...
    if (params<>nil) and (params^.size>=1) then begin
      ...
      if (sizeWithoutOptions = 1) and (arg0^.literalType in [lt_list,lt_intList, lt_realList, lt_numList]) then begin
        context.connector^.postCustomMessage(addRowMessage(newDataRow(list0)));
        exit(newVoidLiteral);
      end;
      if (sizeWithoutOptions = 2) and
        ...
        context.connector^.postCustomMessage(addRowMessage(newDataRow(list1,list0)));
        exit(newVoidLiteral);
      end;
      if (sizeWithoutOptions = 4) and
        ...
        context.connector^.postCustomMessage(addRowMessage(generateRow(P_expressionLiteral(arg0),
                                                                fReal(arg1),
                                                                fReal(arg2),
                                                                int3^.intValue,
                                                                tokenLocation,
...
FUNCTION plot intFuncSignature;
  begin
    ...
    context.connector^.postSingal(mt_plot_clear,C_nilTokenLocation);
...
FUNCTION setOptions intFuncSignature;
  ...
  PROCEDURE matchKey(CONST key:string; CONST value:P_literal);
    PROCEDURE fail;
      begin
        context.connector^.postTextMessage(mt_el2_warning,tokenLocation,
  ...
  begin
    ...
    if allOkay then begin
      ...
      context.connector^.postCustomMessage(postOptionsMessage);
    end;
  end;
  ...
  begin
    ...
    if (params<>nil) and (params^.size>=3) and
      ...
      try
        ...
        context.connector^.postCustomMessage(renderRequest);
      except
...
FUNCTION renderToString_impl intFuncSignature;
  ...
  begin
    ...
    if (params<>nil) and (params^.size>=2) and
      ...
      context.connector^.postCustomMessage(renderRequest^.rereferenced);
...
FUNCTION removePlot_imp intFuncSignature;
  ...
  begin
    ...
    if (params=nil) or (params^.size=0) or
      ...
      context.connector^.postCustomMessage(dropPlotMessage);
      result:=newVoidLiteral;
    end else result:=nil;
  end;
...
FUNCTION drawTextRelativeOrAbsolute(CONST params:P_listLiteral; CONST tokenLocation:T_tokenLocation; VAR context:T_threadContext; CONST abspos:boolean):P_literal;
  ...
  begin
    ...
    if (params<>nil) and (params^.size>=3) and
      ...
      context.connector^.postCustomMessage(postRequest);
      result:=newVoidLiteral;
    end;
  end;
