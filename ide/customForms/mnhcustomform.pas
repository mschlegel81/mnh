  ...
  begin
    ...
    try
      ...
      if state.actionTriggered then begin
        ...
        then tmp:=evaluteExpression(config.action,location,context,recycler,state.actionParameter,nil).literal
        else tmp:=config.action^.evaluate(        location,context,recycler).literal;
      ...
      if config.enabled<>nil then begin
        ...
        state.enabled:=evaluateToBoolean_strict(config.enabled,location,context,recycler);
      ...
      if config.caption<>nil then begin
        ...
        tmp:=config.caption^.evaluate(location,context,recycler).literal;
        if tmp<>nil then begin
...
INITIALIZATION
  builtinFunctionMap.registerRule(GUI_NAMESPACE,'showDialog',@showDialog_impl,ak_binary,[se_alterGuiState,se_input]);
